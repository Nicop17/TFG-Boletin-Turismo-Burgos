...
\item \textbf{Creación del repositorio:} Acceder a GitHub y crear un repositorio completamente nuevo y vacío (sin archivo README ni \ruta{.gitignore}).
    ...
    \begin{itemize}
        ...
        \item \texttt{git commit -m \char`\"Initial commit"}: Registra este estado inicial de los archivos.
        ...
        \item \texttt{git push -u origin main}: Realiza la subida definitiva de todo el código a la nube.
    \end{itemize}
\end{enumerate}
...
Una vez creada la cuenta de GC (entrar con el correo de Google), en la barra de arriba, hay que hacer clic en el botón de proyectos y pulsar el botón \texttt{"Proyecto nuevo"}. Se escribe el nombre del proyecto, se le indica que es un proyecto \texttt{"Sin organización"} (para evitar las restricciones de la organización que tiene una política que impide generar claves) y se da a \texttt{"Crear"}. Una vez creado, el proyecto debería estar seleccionado en la barra de arriba.
...
\begin{enumerate}
    \item \textbf{Crear el Dataset:} En el panel izquierdo de BigQuery (En la pestaña de Explorador clásico), hay que hacer clic en los tres puntos verticales que salen al lado del nombre del proyecto y elegimos la opción \texttt{\char`\"Crear conjunto de datos"}. En el formulario que se abre, se le da un nombre al dataset y se crea al hacer clic en el botón azul de abajo. Importante recordar cambiar los nombres que vienen por defecto tanto del proyecto como del dataset en el config.json por los elegidos en este paso.
    ...
    \item \textbf{Pegar el diseño de las tablas:} Desde el entorno de desarrollo local, se abre el archivo \ruta{database/schema.sql}, se copia la totalidad de su contenido (las sentencias \texttt{CREATE TABLE} de las 6 tablas del sistema) y se pega en el editor de consultas de la web de BigQuery.
    \item \textbf{Comprobar los nombres:} Importante revisar que la primera línea de cada consulta tenga el mismo nombre que le dimos antes al proyecto y al dataset de BigQuery. El formato debe ser siempre: \texttt{`nombre\_proyecto.nombre\_dataset.tabla`}.
...
Para las tablas \texttt{municipalities} y \texttt{categories}, en el menú izquierdo de BigQuery, hacemos clic sobre el nombre que le pusimos al dataset y, en la barra de herramientas de arriba, le damos al botón \texttt{\char`\"Crear tabla"} y rellenamos el formulario con estas opciones:
\begin{itemize}
    \item \textbf{Crear tabla desde:} Seleccionamos la opción \texttt{"Subir"}.
    \item \textbf{Seleccionar archivo:} Buscamos en el equipo el archivo que corresponda (por ejemplo, \ruta{database/municipalities.csv} o \ruta{database/categories.csv}).
    \item \textbf{Formato de archivo:} Seleccionamos \texttt{\char`\"CSV"}.
    \item \textbf{Nombre de la tabla:} Escribimos el nombre exacto de la tabla donde va ese archivo (\texttt{municipalities} o \texttt{categories}).
...
Para la tabla de códigos postales (\texttt{municipality\_postal\_codes}) se deben seguir exactamente los mismos pasos, excepto en el apartado de \textbf{Esquema}. Para esta tabla hay que marcar la casilla de \texttt{\char`\"Editar como texto"} y pegar exactamente lo siguiente: \texttt{id\_municipality:INTEGER,municipality\_\-name:STRING,postal\_code:STRING}. Esto es estrictamente necesario porque BigQuery, al usar la detección automática, interpreta los códigos postales del archivo CSV como valores numéricos (\textit{Integer}). Al intentar subir estos datos como números a una tabla cuyo esquema ya ha sido definido previamente en el \ruta{schema.sql} como una cadena de texto (\textit{String}), la plataforma lanza un error de incompatibilidad de tipos y bloquea la creación de la tabla. Forzando el esquema manual se soluciona este conflicto y se asegura que se respeten los formatos.
...
\begin{enumerate}
    ...
    \item En el apartado de asignar roles, seleccionar en el desplegable el rol de \texttt{Editor de datos BigQuery} para otorgar los permisos de escritura necesarios, y hacer clic en \texttt{"Listo"}.
...
\begin{enumerate}
    ...
    \texttt{APIFY\_TOKEN=token\_copiado}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{Value:} Abrir el archivo \ruta{key.json} en el ordenador local, seleccionar y copiar todo el bloque de texto que contiene el archivo y pegarlo entero en este recuadro.
...
\subsubsection{Visualización en Looker Studio}
Una de las grandes ventajas de la arquitectura del proyecto es que el cuadro de mando de Looker Studio no requiere ningún tipo de ejecución, actualización manual o despliegue de código. Al estar conectado de forma directa y nativa al almacén de datos de Google BigQuery, cualquier inserción, modificación o actualización de reseñas realizada por el scraper se refleja de forma completamente dinámica e instantánea en los gráficos del informe.
